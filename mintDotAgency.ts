import {
  ...
  agencyAndAppConfig,
  userConfig,
  defaultDotAgencyTokenURI,
  defaultAgentTokenURI,
  WrapCoinAddress,
  tokenURIEngineConfig,
} from "./config";
...
import {
  displayNotFundAndExit,
  inputAddress,
  selectWrapAddress,
  selectTokenId,
  inputETHNumber,
  inputMoreThanMinimumValue,
  chooseAgencyNFTWithTokenId,
  getExtraAgencyConfig,
  selectDotAgency,
  inputTokenNumber,
  selectOrInputTokenURIEngineAddress,
} from "./utils/display";
...
import {
  getAgencyStrategy,
  getDotAgencyERC6551AddressByTokenID,
  getERC20Approve,
  getTokenBaseInfo,
  isApproveOrOwner,
} from "./utils/data";
...
export const mintDotAgency = async () => {
  ...
  console.log(
    `.Agency Price is ${chalk.blue(formatEther(nowDotAgencyPrice))} ETH`
  );
  console.log(
    `Your ETH Balance is ${chalk.blue(formatEther(accountBalance))} ETH`
  );
  ...
  if (answer) {
    let dotAgencyName = (
      await input({ message: "Enter .Agency Name: " })
    ).toLowerCase();
    while (!normalName(dotAgencyName)) {
      console.log(
        chalk.red(
          "Name has been registeredThe name can only contain lowercase letters and Arabic numerals."
        )
      );
      dotAgencyName = (
        await input({ message: "Enter .Agency Name: " })
    ...
    const userPrice = await inputETHNumber(
      "Maximum cost available for mint(ETH): ",
      formatEther((nowDotAgencyPrice * BigInt(21)) / BigInt(20))
    );

    const gasX = await input({
      message: "gasPrice: ",
      default: "1",
    });

    const priceNonce = await getPriceNonce(gasX);
    await mintDotAgencyName(dotAgencyName, userPrice, priceNonce, gasX);
  }
};

export const mintDotAgencyWithPriceNonce = async () => {
  const nowDotAgencyPrice = await publicClient.readContract({
    ...dotAgency,
    functionName: "getPrice",
  });
  ...
  if (answer) {
    ...
    const priceNonce = await input({
      message: "Input priceNonce:",
    });

    await mintDotAgencyName(dotAgencyName, userPrice, BigInt(priceNonce), gasX);
  }
};
...
export const deployAppAndAgency = async () => {
  ...
  const { agencyImplementation, appImplementation } = agencyAndAppConfig.find(
    ({ value }) => value === deployAppAndAgencySelect
  )!;

  let inputConfig = await getAgencyConfig(
    agencyImplementation as `0x${string}`,
    appImplementation as `0x${string}`
  );
  let configIsTrue = await confirm({
    message: "Continue Deploy App and Agency?",
  });

  while (!configIsTrue) {
    inputConfig = await getAgencyConfig(
      agencyImplementation as `0x${string}`,
      appImplementation as `0x${string}`
    );
    configIsTrue = await confirm({
      message: "Continue Deploy App and Agency?",
    });
  }

  await deployAgencyAndApp(
    inputConfig.tokenId,
    agencyImplementation as `0x${string}`,
    appImplementation as `0x${string}`,
    inputConfig.config,
    inputConfig.extraAgencyData
  );
...
export const changeDotAgencyTokenURI = async () => {
  ...
  const authorityExist = await isApproveOrOwner(
    dotAgency.address,
    BigInt(dotAgencyTokenId)
  );
  ...
  if (!authorityExist) {
    ...
    const tokenURI = await inputAddress(
      "Enter TokenURI Engine Address: ",
      defaultDotAgencyTokenURI
    );
    const { request } = await publicClient.simulateContract({
      account,
...
const getDotAgencyERC6551Address = async (agencyAddress: `0x${string}`) => {
  ...
  const { result: dotAgencyNFTERC6551Address } =
...
export const rebaseFee = async () => {
  ...
  const dotAgencyNFTERC6551Address = await getDotAgencyERC6551Address(
    agencyAddress
  );

  console.log(
    `.Agency ERC6551 Address: ${chalk.blue(dotAgencyNFTERC6551Address)}`
  );
  ...
  console.log(
    `Reward to claim: ${chalk.blue(
      formatUnits(dotAgencyFee * BigInt(5), decimals)
    )}`
  );

  const answer = await confirm({
    message: "Continue Claim Reward from Agency?",
  });
...
export const wrap = async () => {
  ...
  const { name: tokenName, decimals: tokeDecimals } = await getTokenBaseInfo(
    agencyStrategy[1].currency
  );

  const userBalance = await getUserBalance(agencyStrategy[1].currency);

  const nowAgencyPrice = await getAgentMintPrice(
    agencyAddress,
    agencyStrategy[0]
  );

  console.log(
    `ERC7527 Price is ${chalk.blue(
      formatUnits(nowAgencyPrice[0], tokeDecimals)
    )} ${tokenName}, Fee is ${chalk.blue(
      formatUnits(nowAgencyPrice[1], tokeDecimals)
    )} ${tokenName}`
  );
  console.log(
    `Your Balance is ${chalk.blue(
      formatUnits(userBalance, tokeDecimals)
    )} ${tokenName}`
  );

  displayNotFundAndExit(nowAgencyPrice[0] + nowAgencyPrice[1], userBalance);

  const userSlippagePrice = await inputTokenNumber(
    "Maximum cost available for mint: ",
    tokeDecimals,
    formatUnits(
      ((nowAgencyPrice[0] + nowAgencyPrice[1]) * BigInt(21)) / BigInt(20),
      tokeDecimals
    )
  );
  let agencyTokenName = (
    await input({ message: "Enter ERC7527 Name: " })
  ).toLowerCase();
...
export const unwrap = async () => {
  ...
  const agencyStrategy = await getAgencyStrategy(agencyAddress);
  // const tokenName = await getERC20Name(agencyStrategy[1].currency)
  const { name: tokenName, decimals } = await getTokenBaseInfo(
    agencyStrategy[1].currency
  );
  ...
  console.log(
    `Burn ERC7527 will get ${chalk.blue(
      formatUnits(burnGet[0] - burnGet[1], decimals)
    )} ${tokenName}`
  );
  ...
  if (answer) {
    const agencyTokenId = BigInt(
      await input({ message: "Enter ERC7257 ID: " })
    );
    const authorityExist = await isApproveOrOwner(
      agencyStrategy[0],
      agencyTokenId
...
export const updateAgenctConfig = async () => {
  ...
  console.log(
    boxen(
      `Agency Name: ${chalk.blue(agencyName)}\n` +
        `Currency: ${chalk.blue(tokenName)}\n` +
        `Currency Address: ${chalk.blue(agencySettings[1].currency)}\n` +
        `Base Premium: ${chalk.blue(
          agencySettings[1].basePremium.toString(10)
        )}\n` +
        `Mint Fee Percent: 5%\n` +
        `Burn Fee Percent: 5%\n` +
        `Max Supply: ${chalk.blue(
          agentMaxSupply === BigInt(0) ? "Unlimited" : agentMaxSupply
        )}`,
      { padding: 1 }
    )
  );
...
export const createERC6551Account = async () => {
  ...
  if (!accountBytecode) {
    ...
    console.log(
      `Create ERC6551 Account Hash: ${chalk.blue(createAccountHash)}`
    );
  }
};
...
const existName = async (name: string) => {
  ...
  const subNode = keccak256(
    concat([
      "0xb43dbfc1d2fecc659fffd218f4abb6ed0b35bd3896ba6be21f0ca46fb2102ab1",
      nameHash,
    ])
  );
...
const updateConfig = async (
  tokenId?: { name: string; value: number },
  agency?: { value: string; description: string }
) => {
  if (tokenId) {
...
const getPriceNonce = async (gasX: string) => {
  ...
  console.log(`priceNonce: ${chalk.blue(result)}`);
...
const getAgencyConfig = async (
  ...
  if (userConfig.tokenId.length === 0) {
    ...
    tokenId = Number.parseInt(
      await input({ message: "Enter Your Agency Token ID:" })
    );
  ...
  const currency = await inputAddress(
    "Enter ERC20 address (zero address is ETH):",
    WrapCoinAddress
  );
  // const currencyName = await getERC20Name(currency)
  const { name: currencyName, decimals: tokeDecimals } = await getTokenBaseInfo(
    currency
  );
  const basePremium = parseUnits(
    await input({ message: "Enter Base Premium:", default: "1" }),
    tokeDecimals
  );
  const feeRecipient = getAddress("0x0000000000000000000000000000000000000000");
  const mintFeePercent = 0;
  const burnFeePercent = 0;
  const maxSupply = Number.parseInt(
    await input({
      message: "Enter Max Supply(If set to 0, unlimited supply): ",
      default: "0",
    }),
    10
  );
  ...
  console.log(
    boxen(
      `Currency: ${chalk.blue(currencyName)}\nBase Premium: ${chalk.blue(
        basePremium.toString(10)
      )}\nMint Fee Percent: 5%\nBurn Fee Percent: 5%\nMax Supply: ${chalk.blue(
        maxSupply === 0 ? "Unlimited" : maxSupply
      )}`,
      { padding: 1 }
    )
  );
...
const deployAgencyAndApp = async (
  ...
  const tokenName = userConfig.tokenId.find(
    ({ value }) => value === tokenId
  )!.name;
...
const getAgenctBurnPrice = async (
  agencyAddress: `0x${string}`,
  appAddress: `0x${string}`
) => {
  const totalSupply = await getAgencyTotalSupply(appAddress);
...
const wrapAgency = async (
  name: string,
  price: bigint,
  agencyAddress: `0x${string}`,
  tokenAddress: `0x${string}`
) => {
  const toAddress = await inputAddress(
    "Enter ERC7527 Receiver Address: ",
    account.address
  );
  ...
  if (tokenAddress === "0x0000000000000000000000000000000000000000") {
    ...
    console.log(
      `Approve Value: ${chalk.blue(formatUnits(userApproveValue, decimals))}`
    );
    if (userApproveValue < price) {
      const userNewApprove = await inputTokenNumber(
        "Enter New Approve Value: ",
        decimals,
...
const unwrapAgency = async (
  ...
  const toAddress = await inputAddress(
    `Enter ${tokenName} Receiver Address: `,
    account.address
  );
  const args = encodeAbiParameters(
    [
...
export const claimLockWrapCoin = async () => {
  ...
  if (answer) {
    const dotAgencyNFTERC6551Address =
      await getDotAgencyERC6551AddressByTokenID(dotAgencyTokenId);
    console.log(
      `Wrap Recipient ERC6551 Address: ${chalk.blue(
        dotAgencyNFTERC6551Address
      )}`
    );
...
export const renouncePush = async () => {
  const agencyAddress = await selectWrapAddress(userConfig);
  const [isRenounceForceApprove, isRenounceForceCancel, dotAgencyTokenId] =
  ...
  if (!isRenounceForceApprove.result) {
    const renounceForceApprove = await confirm({
      message: "Renounce Force Approve?",
    });
    if (renounceForceApprove) {
      ...
      const renounceForceApproveHash = await walletClient.writeContract(
        request
      );
      console.log(
        `Renounce Force Approve Hash: ${chalk.blue(renounceForceApproveHash)}`
      );
    }
  }

  if (!isRenounceForceCancel.result) {
    const renounceForceCancel = await confirm({
      message: "Renounce Force Cancel?",
    });
    if (renounceForceCancel) {
      ...
      console.log(
        `Renounce Force Cancel Hash: ${chalk.blue(renounceForceCancelHash)}`
      );
    }
  }
